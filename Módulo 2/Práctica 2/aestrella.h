...
float heuristica(int A, int L){
   return abs((sqrt(pow( nodes[L].x - nodes[A].x ,2) + pow( nodes[L].y - nodes[A].y ,2))));
}

void aestrella_algorithm(int D ,int L)
{
	int menor,flagOnce;
	int contador=0;
		...
		for(int j=0 ; j < nodes[D].num_conections; j++  )
   		{
   			if(nodes[nodes[D].conections[j].node].flag == 'N')
   				{
   					nodes[nodes[D].conections[j].node].acumulado = nodes[D].acumulado + nodes[D].conections[j].cost + heuristica(D,L);
   			...
   			{
   				if( nodes[nodes[D].conections[j].node].acumulado > nodes[D].acumulado + nodes[D].conections[j].cost + heuristica(D,L))
   				{
   					nodes[nodes[D].conections[j].node].acumulado = nodes[D].acumulado + nodes[D].conections[j].cost+ heuristica(D,L);
   					nodes[nodes[D].conections[j].node].parent = D;
   				}
...
int aestrella(float rx ,float ry ,float lx ,float ly, char *world_name,step *steps )
{
   char archivo[150];
   int i;
   int start = 0;
...
*/
   aestrella_algorithm (goal ,start); // se pasan alreves para no tener que voltear la lista resultante
   int padre = start;
